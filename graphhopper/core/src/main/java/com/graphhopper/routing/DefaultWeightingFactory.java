...
public class DefaultWeightingFactory implements WeightingFactory {
    ...
    public Weighting createWeighting(Profile profile, PMap requestHints, boolean disableTurnCosts) {
        ...
        if (CustomWeighting.NAME.equalsIgnoreCase(weightingStr)) {
            ...
            DecimalEncodedValue enc = GraphHopper.pollutionEnc;

            if (enc == null) {
                throw new IllegalStateException("pollutionEnc no ha sido inicializado. " +
                        "Asegúrate de haberlo añadido en buildEncodingManager.");
            }

            weighting = new CustomWeighting(enc, turnCostProvider, parameters);
