...
public class OSMReader {
    ...
    protected void addEdge(int fromIndex, int toIndex, PointList pointList, ReaderWay way, List<Map<String, Object>> nodeTags) {
        ...
        double lat1 = na.getLat(edge.getBaseNode());
        double lon1 = na.getLon(edge.getBaseNode());

        double lat2 = na.getLat(edge.getAdjNode());
        double lon2 = na.getLon(edge.getAdjNode());

        // Dentro de OSMReader.java
        double pollution = pollutionService.getPollutionMidpoint(lat1, lon1, lat2, lon2);

        if (GraphHopper.pollutionEnc != null) {
            edge.set(GraphHopper.pollutionEnc, pollution);
        } else {
            // Si esto sale en el log, es que el EncodedValue no se registró correctamente
            LOGGER.error("ERROR: pollutionEnc es NULL en OSMReader");
        }
