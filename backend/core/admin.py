...
from .models import (
    Route, Usuari, Titol, UsuariTitol, PlaEntrenament,
    TemplateExercici, Exercici, UsuariRuta, Amistat,
    Conversa, Missatge, Forum, ForumFavorit,
    BicingEstacio, AirQualityHistoric
)
...
class BicingEstacioAdmin(admin.ModelAdmin):
    list_display = ('station_id', 'name', 'capacity', 'bikes_available', 'docks_available')
    search_fields = ('name',)
...
class ExerciciAdmin(admin.ModelAdmin):
    list_display = ('id', 'usuari', 'dataInici', 'completat', 'distanciaObjectiu', 'distanciaFeta')
