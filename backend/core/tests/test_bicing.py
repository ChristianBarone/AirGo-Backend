...
class TestBicingAPI:
    ...
    def test_extract_station_name_logic(self):
        ...
        names = [{"language": "ca", "text": "Català"}, {"language": "es", "text": "Cast"}]
    ...
    def test_get_bicing_near_integration(self, mock_status, mock_info):
        mock_info.return_value = [{"station_id": 1, "name": "Test", "lat": 41.0, "lon": 2.0}]
        mock_status.return_value = {"1": {"num_bikes_available_types": {"mechanical": 5}, "status": "IN_SERVICE"}}
    ...
    def test_bicing_fallback_database(self):
        BicingEstacio.objects.create(
            station_id=123, name="Estacio Local", lat=41.3, lon=2.1,
        ...
        with patch('core.services.bicing.requests.get', side_effect=requests.exceptions.RequestException):
